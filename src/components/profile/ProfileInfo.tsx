...
export const ProfileInfo = ({
  ...
  return <Card className="border-primary/10 shadow-lg hover:shadow-primary/5 transition-all duration-300 backdrop-blur-sm bg-gradient-to-br from-[#1A1F2C]/95 to-[#1A1F2C]/80 overflow-hidden relative">
        ...
        <div className="space-y-2 group">
          ...
          <div className={`flex ${isMobile ? 'flex-col' : 'gap-4'} items-start`}>
            <div className={`${isMobile ? 'w-full mb-3' : 'flex-grow'} flex gap-2 items-center relative overflow-hidden rounded-lg transition-all duration-300`}>
              <div className="absolute inset-0 bg-gradient-to-r from-primary/5 via-purple-500/5 to-primary/5 opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
              <Input value={userData?.wallet_address || ''} readOnly className="bg-white/5 font-mono text-sm border-white/10 group-hover:border-primary/30 transition-colors pl-10 pr-16 truncate text-white/90 h-12 w-full" placeholder="No wallet address generated" />
