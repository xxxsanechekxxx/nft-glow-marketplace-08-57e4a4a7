...
export const TransactionHistory = ({ transactions: initialTransactions }: TransactionHistoryProps) => {
  ...
  const getTypeIcon = (type: string, isFrozenExchange: boolean) => {
    switch(type) {
      case 'deposit':
        return <ArrowDownCircle className="w-4 h-4 text-emerald-500 flex-shrink-0" />;
      case 'withdraw':
        return <ArrowUpCircle className="w-4 h-4 text-rose-500 flex-shrink-0" />;
      case 'purchase':
        return <ShoppingBag className="w-4 h-4 text-blue-500 flex-shrink-0" />;
      case 'sale':
        return <ShoppingBag className="w-4 h-4 text-emerald-500 flex-shrink-0" />;
      case 'exchange':
        return <ArrowRightLeft className={`w-4 h-4 ${isFrozenExchange ? 'text-amber-500' : 'text-indigo-500'} flex-shrink-0`} />;
      default:
        return null;
    }
  };

  const getStatusText = (status: string, isFrozen: boolean, isFrozenExchange: boolean) => {
    ...
    if (isMobile) {
      if (isFrozenExchange) return "Frozen Exchange";
      if (isFrozen) return "Frozen";
      return status === 'completed' ? 'Completed' : 'Pending';
    }
    
    if (isFrozenExchange) return "Frozen Exchange";
    if (isFrozen) return "Frozen until";
  ...
  return (
    <Card className="border-primary/10 shadow-lg hover:shadow-primary/5 transition-all duration-300 backdrop-blur-sm bg-[#1A1F2C]/90 mt-6">
      <CardHeader className="flex flex-col sm:flex-row items-start sm:items-center justify-between pb-2 space-y-3 sm:space-y-0 px-4">
        <CardTitle className="text-xl font-bold bg-gradient-to-r from-white to-white/70 bg-clip-text text-transparent flex items-center gap-2">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 w-full sm:w-auto">
          <div className="relative w-full sm:w-48">
            ...
            <Input
              ...
              className="pl-7 h-8 bg-background/50 border-primary/20 focus:border-primary/50 w-full text-xs"
          ...
          <div className="flex flex-wrap gap-1 w-full sm:w-auto">
            <Button 
              variant="outline" 
              size="sm" 
              className={`px-2 h-8 text-xs border-primary/20 ${filterType === null ? 'bg-primary/20 text-primary' : 'bg-background/50'}`}
            ...
            <Button 
              variant="outline" 
              size="sm" 
              className={`px-2 h-8 text-xs border-primary/20 ${filterType === 'deposit' ? 'bg-emerald-500/20 text-emerald-500' : 'bg-background/50'}`}
              onClick={() => {
            ...
            >
              ...
              <span className={isTiny ? "" : ""}>Deposits</span>
            </Button>
            <Button 
              variant="outline" 
              size="sm" 
              className={`px-2 h-8 text-xs border-primary/20 ${filterType === 'exchange' ? 'bg-indigo-500/20 text-indigo-500' : 'bg-background/50'}`}
              onClick={() => {
            ...
            >
              ...
              <span className={isTiny ? "" : ""}>Exchanges</span>
            </Button>
          </div>
        </div>
      </CardHeader>
            ...
            <Table>
              <TableHeader>
                <TableRow className="hover:bg-primary/5 border-b border-primary/10">
                  <TableHead className="text-xs text-muted-foreground font-medium w-[25%]">Date</TableHead>
                  <TableHead className="text-xs text-muted-foreground font-medium w-[15%] text-center">
                    <span className={isTiny ? "hidden" : ""}>Type</span>
                  </TableHead>
                  <TableHead className="text-xs text-muted-foreground font-medium w-[25%]">Amount</TableHead>
                  <TableHead className="text-xs text-muted-foreground font-medium w-[35%] text-center sm:text-left">
                    <span className={isTiny ? "hidden" : ""}>Status</span>
                  </TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {filteredTransactions.map(transaction => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell className="px-2 py-2 text-xs text-center sm:text-left">
                        <div className="flex items-center justify-center sm:justify-start">
                          <span className={`inline-block px-2 py-1 rounded-full font-medium whitespace-nowrap ${
                            ...
                            {getStatusText(transaction.status, !!transaction.frozen_until, isFrozenExchange)}
                          </span>
                        </div>
                      </TableCell>
                    </TableRow>
            ...
            >
              {loading ? (
                <div className="flex items-center gap-2 text-muted-foreground">
                  ...
                  <span className="text-xs">Loading more transactions...</span>
                </div>
              ) : hasMore ? (
                <Button 
                  variant="outline" 
                  size="sm" 
                  className="px-2 h-8 border-primary/20 text-xs"
                ...
                <span className="text-xs text-muted-foreground">No more transactions</span>
              )}
            </div>
          </ScrollArea>
        ) : (
