...
import { Check, X, PencilLine, ShoppingCart, Tag } from "lucide-react";
  ...
  return (
    <>
          ...
          <div className="relative rounded-xl overflow-hidden bg-background/60 backdrop-blur-sm border border-primary/10 shadow-lg hover:shadow-primary/5 transition-all duration-700 h-full">
            {isForSale && (
              <div className="absolute top-3 right-3 z-10">
                <Badge 
                  variant="outline" 
                  className="bg-emerald-500/20 text-emerald-300 border-emerald-500/30 backdrop-blur-md px-2 py-1 text-xs"
                >
                  For Sale
                </Badge>
              </div>
            ...
            <div className="p-3 sm:p-4 md:p-5 space-y-1 sm:space-y-2">
              ...
              <div className="flex items-center justify-between mt-2 sm:mt-3 md:mt-4 pt-2 border-t border-primary/10">
                {isEditingPrice ? (
                  <div className="flex items-center gap-1 sm:gap-2 flex-1" onClick={(e) => e.stopPropagation()}>
                    <Input 
                      ...
                      className="h-7 sm:h-8 md:h-9 w-16 sm:w-20 md:w-24 text-xs sm:text-sm"
                      min="0.01"
                      step="0.01"
                  ...
                  <div className="flex gap-1 sm:gap-2" onClick={(e) => e.stopPropagation()}>
                    {isForSale ? (
                      <>
                        {!isEditingPrice && (
                          <Button 
                            ...
                            size="sm"
                            variant="outline"
                            className="h-8 px-2 flex gap-1 items-center bg-primary/10 hover:bg-primary/20 border-primary/20 text-xs"
                            title="Edit Price"
                          >
                        ...
                        <Button 
                          ...
                          size="sm"
                          variant="outline"
                          className="h-8 px-2 flex gap-1 items-center bg-destructive/10 hover:bg-destructive/20 border-destructive/20 text-destructive text-xs"
                          title="Cancel Sale"
                        >
                          <X className="h-3 w-3" />
                      ...
                      <Button 
                        onClick={handleSell} 
                        size="sm"
                        className="relative overflow-hidden transition-all duration-500 hover:scale-105 hover:shadow-lg hover:shadow-primary/20 px-2 sm:px-3 py-1 h-8 text-xs flex items-center gap-1"
                      >
                        <Tag className="h-3 w-3 sm:h-4 sm:w-4" />
                        <span className="relative z-10">Sell NFT</span>
      ...
      <AlertDialog open={showCancelDialog} onOpenChange={setShowCancelDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Cancel NFT Sale</AlertDialogTitle>
            <AlertDialogDescription>
              Are you sure you want to cancel the sale of "{name}"? This will remove the NFT from all marketplaces.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel onClick={(e) => e.stopPropagation()}>No, keep it listed</AlertDialogCancel>
            <AlertDialogAction 
              onClick={(e) => {
                e.stopPropagation();
                handleCancelSale();
