...
const Profile = () => {
  ...
  const [isExchangeAvailableDialogOpen, setIsExchangeAvailableDialogOpen] = useState(false);
  const [exchangeDirection, setExchangeDirection] = useState<'eth_to_usdt' | 'usdt_to_eth'>('eth_to_usdt');
  const [exchangeAvailableAmount, setExchangeAvailableAmount] = useState("");
  const handleAvailableExchange = (e: React.FormEvent) => {
    e.preventDefault();
    const exchangeAmountNum = parseFloat(exchangeAvailableAmount);
    if (exchangeAmountNum <= 0) {
      toast({
        title: "Error",
        description: "Please enter a valid amount greater than 0",
    ...
    try {
      ...
      toast({
        title: "Exchange Requested",
        description: `Your exchange request for ${exchangeAvailableAmount} ${exchangeDirection === 'eth_to_usdt' ? 'ETH to USDT' : 'USDT to ETH'} has been submitted`
      });
      setExchangeAvailableAmount("");
      setIsExchangeAvailableDialogOpen(false);
    } catch (error) {
      toast({
        title: "Error",
        description: "Failed to process exchange. Please try again.",
                ...
                <div className="space-y-2 group">
                  ...
                  <div className="flex gap-4 items-start">
                    <div className="flex-grow flex gap-2 items-center relative overflow-hidden rounded-lg transition-all duration-300 group-hover:shadow-lg">
                      <div className="absolute inset-0 bg-gradient-to-r from-primary/5 via-purple-500/5 to-primary/5 opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
                      <Input value={userData?.wallet_address || ''} readOnly className="bg-background/50 font-mono text-
