...
const Profile = () => {
  ...
  const handleExchangeHoldBalance = async () => {
    try {
      const amountToExchange = parseFloat(exchangeAmount);
      const frozenBalance = parseFloat(userData?.frozen_balance || "0");
      ...
      if (amountToExchange > frozenBalance) {
        toast({
          title: "Insufficient Funds",
          description: `Your hold balance (${frozenBalance} ETH) is less than the requested exchange amount`,
          variant: "destructive",
        });
        return;
      }
      
      // Convert ETH to USDT (using a fixed rate for demo purposes - in production use an API)
      const ethToUsdtRate = 3000; // 1 ETH = 3000 USDT
      const usdtAmount = amountToExchange * ethToUsdtRate;
      
      // Update user's balances
      const { error } = await supabase
        .from('profiles')
        .update({
          frozen_balance: frozenBalance - amountToExchange,
          usdt_balance: parseFloat(userData?.usdt_balance || "0") + usdtAmount
        })
        .eq('user_id', user?.id);
      ...
      setUserData(prev => {
        ...
        return {
          ...prev,
          frozen_balance: (frozenBalance - amountToExchange).toString(),
          usdt_balance: (parseFloat(prev.usdt_balance || "0") + usdtAmount).toString()
        };
      });
        ...
        .insert([
          {
            ...
            status: 'completed',
            item: `Exchanged ${amountToExchange} ETH to ${usdtAmount.toFixed(2)} USDT`
          }
        ]);
      
      toast({
        title: "Exchange Successful",
        description: `Exchanged ${amountToExchange} ETH to ${usdtAmount.toFixed(2)} USDT`,
      });
